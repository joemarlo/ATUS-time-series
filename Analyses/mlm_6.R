...
hamming_clusters <- read_csv(file = "Analyses/Hamming/Hamming_6_clusters.csv", 
                             col_types = cols(hamming_cluster = col_integer()))
levenshtein_clusters <- read_csv(file = "Analyses/Levenshtein/Levenshtein_6_clusters.csv", 
...
final_df <- clusters_df %>% 
  ...
  mutate(across(contains("cluster"), ~ {
    ...
    x <- factor(x, levels = c("Cluster 1", as.character(2:6))) 
  }))
...
mlm_models %>% 
  ...
  mutate(description = factor(description, 
                              levels = c('Day workers', 'Day workers - late', 'Night workers', 'Students', "Leisure'ers", "Leisure'ers - 2", 'Householders', 'Uncategorized'))) %>%
